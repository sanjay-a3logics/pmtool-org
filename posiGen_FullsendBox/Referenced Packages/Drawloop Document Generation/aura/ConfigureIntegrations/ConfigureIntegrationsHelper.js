    ...
    getAvailableIntegrations : function(component) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var iiSet = new Set(existingTypes);
                var singleIntegrations = ['Box', 'DDP API', 'DocuSign', 'e-SignLive', 'Lob'];
                var availableIntegrations = [];
                for (var i in singleIntegrations) {
                    var type = singleIntegrations[i];
                    if (!iiSet.has(type)) {
    ...
    updateValues : function(component, values) {
        ...
        if (integrationData['Loop__Type__c'] == 'Office 365') {
            ...
            for (var i in values) {
                ...
                el.href = values[i]['Loop__Base_URL__c'];
            ...
            var el = document.createElement('a');
            el.href = values[0]['Loop__Base_URL__c'];
            var intBaseUrl = el.protocol + '//' + el.host;
            ...
            if (response.getState() === "SUCCESS") {
                ...
                if (parsedResponse.isSuccess) {
                    ...
                    if (parsedResponse.errors) {
                        ...
                        for (i = 0; i < duplicationErrors.length; i ++) {
