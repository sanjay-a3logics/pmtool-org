({
    ...
    onDoneRendering : function(component, event, helper) {
        ...
        if (selectedDdpAdmins.length > 0) {
            ...
            for (var i = 0; i < selectedDdpAdmins.length; i ++) {
                for (var j = 0; j < ddpAdmins.length; j ++) {
                    if (ddpAdmins[j].Id == selectedDdpAdmins[i].Id) {
                        helper.addSelectedUI(selectedDdpAdmins[i]);
                    }
                }
            }
        }
        ...
        if (selectedDdpUsers.length > 0) {
            ...
            for (var i = 0; i < selectedDdpUsers.length; i ++) {
                for (var j = 0; j < ddpUsers.length; j ++) {
                    if (ddpUsers[j].Id == selectedDdpUsers[i].Id) {
                        helper.addSelectedUI(selectedDdpUsers[i]);
                    }
                }
            }
        }
        ...
        action.setCallback(this, function(response) {
            if (response.getState() === "SUCCESS") {
                ...
                if (parsedResponse.isSuccess) {
                    ...
                    for (var i = 0; i < stagedUsers.length; i ++) {
                        if (stagedUsers[i].DesiredStatus == 'none' && (stagedUsers[i].ProfileId == component.get("v.selectedProfile").Id || component.get("v.selectedProfile").Id == 'allUsers')) {
                            availableUsers.push(stagedUsers[i]);
                        }
                    }
    ...
    availableUserClicked : function(component, event, helper) {
        ...
        for (var i = 0; i < selectedUsers.length; i ++) {
            if (selectedUsers[i].Id == clickedUser.Id) {
        ...
        if (selectedUsers.length > 0) {
            ...
            if (isSandbox || hasSiteWideLicense || usedLicenses + selectedUsers.length <= allowedLicenses) {
                ...
                for (var i = 0; i < selectedUsers.length; i ++) {
                    ...
                    for (var j = 0; j < availableUsers.length; j ++) {
                        if (availableUsers[j].Id == selectedUser.Id) {
    ...
    removeAdmin : function(component, event, helper) {
        ...
        if (selectedUsers.length > 0) {
            ...
            for (var i = 0; i < selectedUsers.length; i ++) {
                ...
                for (var j = 0; j < ddpAdmins.length; j ++) {
                    if (ddpAdmins[j].Id == selectedUser.Id) {
                        ddpAdmins.splice(j, 1);
                        break;
                    }
                }
    ...
    removeUser : function(component, event, helper) {
        ...
        if (selectedUsers.length > 0) {
            for (var i = 0; i < selectedUsers.length; i ++) {
                ...
                for (var j = 0; j < ddpUsers.length; j ++) {
                    if (ddpUsers[j].Id == selectedUser.Id) {
                        ddpUsers.splice(j, 1);
                        break;
                    }
                }
        ...
        action.setCallback(this, function(response) {
            ...
            if (response.getState() === "SUCCESS") {
                ...
                if (parsedResponse.isSuccess) {
                    ...
                    var selectedDdpAdmins = component.get("v.selectedDdpAdmins");
                    for (var i = 0; i < selectedDdpAdmins.length; i ++) {
                        helper.removeSelectedUI(selectedDdpAdmins[i]);
                    }
                    component.set("v.selectedDdpAdmins", []);
                    
                    var selectedDdpUsers = component.get("v.selectedDdpUsers");
                    for (var i = 0; i < selectedDdpUsers.length; i ++) {
                        helper.removeSelectedUI(selectedDdpUsers[i]);
                    }
                    component.set("v.selectedDdpUsers", []);
                    
                    var stagedUsers = component.get("v.stagedUsers");
                    for (var i = 0; i < stagedUsers.length; i ++) {
                        stagedUsers[i].CurrentStatus = stagedUsers[i].DesiredStatus;
                    }
