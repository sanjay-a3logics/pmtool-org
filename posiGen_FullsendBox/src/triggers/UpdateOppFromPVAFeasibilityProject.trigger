trigger UpdateOppFromPVAFeasibilityProject on Project__c (after update) 
{   Trigger_Setting__c ts= Trigger_Setting__c.getInstance();
    if(!ts.Project_Trigger__c){     
        // Grab all related projects 
        List<Project__c> projsWithSavings = [SELECT Id, Result__c, Marked_Completed_On_Date__c, Opportunity__c, Opportunity__r.Id FROM Project__c WHERE Project__c.Record_Type_Name__c
