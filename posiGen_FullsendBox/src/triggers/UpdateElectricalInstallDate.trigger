trigger UpdateElectricalInstallDate on Project__c (after update)
{
// Grab all related projects 
    List<Project__c> projsWithSavings = [SELECT Id, PVI_Electrical_Installed_Date__c, Opportunity__c, Opportunity__r.Id FROM Project__c WHERE Project__c.Record_Type_Name__c
...
/*{
    ...
    {         
        ...
        if( project.PVI_Electrical_Installed_Date__c != null && oldProject.PVI_Electrical_Installed_Date__c == null  )
        {
            //Initialise an Opportunity sObject using the id stored in the updated project
            Opportunity relatedOpp = [SELECT Id,PVI_Electrical_Installed_Date__c FROM Opportunity WHERE ID = :project.Opportunity__c LIMIT 1]; //Add the fieldname of the date field you want to update after the Id in the select statement
           
            //Assign project's *date field* value to the Opportunity's *date field* value
            relatedOpp.PVI_Electrical_Installed_Date__c = project.PVI_Electrical_Installed_Date__c; //NOT ACTUAL FIELD VALUES. CHANGE TO ACTUAL FIELD VALUES OR ELSE!!!!!
